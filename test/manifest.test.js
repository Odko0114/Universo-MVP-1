...
test("write() then read() round-trips fetched_at, count, checksum and extra fields", () => {
  const original = fs.existsSync(manifest.FILE)
    ? fs.readFileSync(manifest.FILE, "utf8")
    : null;
  try {
    const entry = manifest.write("__test_source__", [{ a: 1 }, { a: 2 }], {
      ref_year: 2099,
    });
