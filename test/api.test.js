...
test("GET /api/universities is rate limited (previously had none at all)", async () => {
  ...
  let last;
  for (let i = 0; i < 180; i++)
    last = await fetch(base + "/api/universities?limit=1", { headers });
  assert.equal(
    last.status,
    200,
    "the 180th request within the window still succeeds",
  );
...
test("GET /api/universities/:id is rate limited (previously had none at all)", async () => {
  const id = (await req("GET", "/api/universities?limit=1")).json
    .universities[0].id;
  const headers = { "X-Forwarded-For": "rl-test-detail-1" };
  let last;
  for (let i = 0; i < 120; i++)
    last = await fetch(`${base}/api/universities/${id}`, { headers });
  assert.equal(
    last.status,
    200,
    "the 120th request within the window still succeeds",
  );
...
test("for-universities stat counters render the real number, never a bare 0, and match /discover", async () => {
  ...
  assert.ok(
    !page.includes("{{TOTAL}}") && !page.includes("{{VERIFIED}}"),
    "no unreplaced token",
  );
  assert.match(
    page,
    new RegExp(`data-count="${verified}"`),
    "verified count is injected live",
  );
  ...
  assert.ok(
    !page.includes(`>${total.toLocaleString("en-US")}</div>`),
    "directory total must not appear",
  );
});
...
test("/for-universities carries a data-handling section and no un-evidenced claims", async () => {
  ...
  assert.ok(
    !/EU-hosted|Frankfurt region/.test(page),
    "no unverified EU-residency claim",
  );
  ...
  assert.ok(
    !page.includes("EXACT_NUMBER_OF_COMPLETED_CONVERSATIONS"),
    "stale stat placeholder removed",
  );
  assert.ok(
    !page.includes("FOUNDER_QUOTE_REWRITE"),
    "stale quote placeholder removed",
  );
...
test("PATCH /api/me/profile stores matching fields and flips profile_completed", async () => {
  ...
  assert.equal(
    disc.json.sort,
    "match",
    "profiled student defaults to fit ranking",
  );
  assert.ok(
    disc.json.universities.some(
      (u) => Array.isArray(u.match_reasons) && u.match_reasons.length,
    ),
  );
...
test("signed-out /admin HTML leaks nothing: no setup command, no hint copy, no export URL", async () => {
  ...
  assert.ok(
    !/create-admin/.test(r.text),
    "setup command must not be in public HTML",
  );
  assert.ok(!/Contact the founder/.test(r.text), "no provisioning hint copy");
  assert.ok(
    !/subscribers\.csv/.test(r.text),
    "export URL is injected only after auth",
  );
...
test("every /api/admin/* and partner route rejects unauthenticated access with JSON 401", async () => {
  const adminRoutes = [
    ...
    "/api/admin/data-quality/records",
  ];
...
test("university profile pages stay public for anonymous visitors (SEO surface)", async () => {
  const anyId = (await req("GET", "/api/universities?limit=1")).json
    .universities[0].id;
...
test("unresearched tuition is omitted entirely and replaced by an honest register notice", async () => {
  ...
  const eter = (await req("GET", "/api/universities?source=eter&limit=1")).json
    .universities[0];
  const full = (await req("GET", `/api/universities/${eter.id}`)).json
    .university;
  assert.equal(full.tuition_source, "country_estimate");

  const ssrPage = await req("GET", `/university/${eter.id}`);
  assert.ok(
    !/Tuition \(intl\)<\/dt><dd>~?€/.test(ssrPage.text),
    "no fabricated € figure in SSR facts",
  );
  assert.ok(
    !/Check official site/.test(ssrPage.text),
    "no pseudo-answer in the tuition slot",
  );
  assert.match(
    ssrPage.text,
...
test("thin register profiles are noindex,follow; verified profiles stay indexable", async () => {
  ...
  assert.match(
    ...
    "thin page must be noindex but still followed",
  );

  const verified = await req("GET", "/university/tum"); // curated + verified
  assert.ok(
    !/name="robots"/.test(verified.text),
    "verified profile must stay indexable",
  );
});
...
test("sitemap lists only verified profiles, not the full register", async () => {
  ...
  assert.equal(
    count,
    filters.counts.verified,
    "one sitemap entry per verified profile",
  );
...
test("outbound university links are https and truncated register names are corrected", async () => {
  const rwth = (await req("GET", "/api/universities/eter-de0069")).json
    .university;
  assert.equal(rwth.name, "RWTH Aachen University");
  assert.equal(
    rwth.name_register,
    "Aachen University",
    "original register name kept for provenance",
  );
...
test("robots.txt disallows operational surfaces and references the sitemap", async () => {
  ...
  for (const p of ["/admin", "/partners", "/saved", "/account"])
    assert.match(r.text, new RegExp(`Disallow: ${p}`));
...
test("robots.txt explicitly welcomes AI crawlers on public pages (with the same private-surface guards)", async () => {
  ...
  assert.ok(
    (r.text.match(/Disallow: \/account/g) || []).length >= 2,
    "private surfaces guarded in every group",
  );
...
test("GET /me/journey assembles the dashboard and gains match picks once a profile is set", async () => {
  ...
  assert.equal(
    before.json.picks.length,
    0,
    "no ranked picks without a profile",
  );
  assert.ok(
    Array.isArray(before.json.next_actions) &&
  ...
  await req("PATCH", "/api/me/profile", {
    fields_of_interest: ["Computer Science & IT"],
    degree_level: "Master",
    budget_max_eur_year: 8000,
  });
  ...
  assert.ok(
    after.json.completeness.percent > before.json.completeness.percent,
    "completeness rose",
  );
  assert.ok(after.json.picks.length >= 1, "ranked picks now returned");
  assert.ok(
    after.json.picks[0].match_reasons &&
      after.json.picks[0].match_reasons.length,
    "picks carry a why reason",
  );

  // Dream Plan payload present and honest.
  assert.ok(
    Array.isArray(after.json.readiness) && after.json.readiness.length >= 3,
    "readiness dimensions present",
  );
  assert.ok(
    after.json.next_best_action && after.json.next_best_action.title,
    "a next best action is surfaced",
  );
  assert.ok(
    Array.isArray(after.json.documents) && after.json.documents.length === 6,
    "document checklist present",
  );
  assert.ok(
    after.json.documents.every((d) => d.done === false),
...
test("application status: requires the uni saved, validates status, persists, rolls up, clears on unsave", async () => {
  ...
  const bad = await req("POST", `/api/me/saved/${id}/status`, {
    status: "enrolled",
  });
...
test("POST /me/milestone: auth-gated, validates the key, persists, and reflects in the timeline", async () => {
  const unauth = await fetch(base + "/api/me/milestone", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ key: "arrived", done: true }),
  });
  assert.equal(unauth.status, 401);

  const testAddr = `milestone_${Date.now()}@example.com`;
  await req("POST", "/api/auth/register", {
    full_name: "Milestone Flow",
    email: testAddr,
    password: "password123",
    consent: true,
  });
  ...
  const auto = await req("POST", "/api/me/milestone", {
    key: "profile_set",
    done: true,
  });
  assert.equal(auto.status, 400, "auto stages cannot be toggled by the client");
  const bogus = await req("POST", "/api/me/milestone", {
    key: "not_a_stage",
    done: true,
  });
...
test("Dream Plan: dream fields and document checklist persist, drive readiness, and are auth-gated", async () => {
  const clean = await fetch(base + "/api/me/dream", {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: "{}",
  });
  assert.equal(clean.status, 401);
  const cleanDoc = await fetch(base + "/api/me/document", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ key: "transcript", done: true }),
  });
  assert.equal(cleanDoc.status, 401);

  const testAddr = `dream_${Date.now()}@example.com`;
  await req("POST", "/api/auth/register", {
    full_name: "Dream Flow",
    email: testAddr,
    password: "password123",
    consent: true,
  });
  ...
  const dream = await req("PATCH", "/api/me/dream", {
    target_intake: "Fall 2027",
    career_goal: "Become a doctor",
  ...
  const badIntake = await req("PATCH", "/api/me/dream", {
    target_intake: "whenever",
    career_goal: "",
    scholarship_required: false,
  });
  assert.equal(
    badIntake.json.student.target_intake,
    "",
    "invalid intake is rejected, not stored",
  );
  ...
  await req("PATCH", "/api/me/dream", {
    target_intake: "Fall 2027",
    scholarship_required: true,
  });
  j = await req("GET", "/api/me/journey");
  assert.ok(
    j.json.readiness.some((r) => r.key === "scholarship"),
    "scholarship readiness appears when required",
  );

  // Documents: unknown rejected, valid persists and lifts document readiness.
  const bad = await req("POST", "/api/me/document", {
    key: "not_a_doc",
    done: true,
  });
  assert.equal(bad.status, 400);
  const before = (await req("GET", "/api/me/journey")).json.readiness.find(
    (r) => r.key === "documents",
  ).score;
  ...
  // Unmark clears it.
  const un = await req("POST", "/api/me/document", {
    key: "transcript",
    done: false,
  });
...
test("auth round-trip: register → me → save → export → delete", async () => {
  ...
  assert.equal(
    "password_hash" in reg.json.student,
    false,
...
test("POST /me/logout-everywhere revokes this session AND every previously issued token", async () => {
  ...
  const stale = await fetch(base + "/api/auth/me", {
    headers: { Cookie: `uv_token=${staleToken}` },
  });
...
test("POST /me/logout-everywhere requires authentication", async () => {
  const clean = await fetch(base + "/api/me/logout-everywhere", {
    method: "POST",
  }); // no cookies
  assert.equal(clean.status, 401);
});
...
test("POST /auth/verify-email is rate limited (found missing during security review)", async () => {
  const headers = {
    ...
    "X-Forwarded-For": "rl-test-verify-email",
  };
  const body = JSON.stringify({ token: "a".repeat(64) });
  let last;
  for (let i = 0; i < 20; i++)
    last = await fetch(base + "/api/auth/verify-email", {
      method: "POST",
      headers,
      body,
    });
  assert.equal(
    last.status,
    400,
    "the 20th attempt is still processed (just rejected as an unknown token)",
  );
  const blocked = await fetch(base + "/api/auth/verify-email", {
    method: "POST",
    headers,
    body,
  });
...
test("email verification: full flow, plus invalid/expired tokens rejected", async () => {
  ...
  emailService.sendVerificationEmail = (student, link) => {
    capturedLink = link;
  ...
  try {
    ...
    assert.equal(reg.status, 201);
    assert.equal(
      reg.json.student.email_verified,
      false,
      "unverified at registration",
    );
    assert.ok(
      capturedLink,
      "verification link was generated and handed to the email service",
    );
    ...
    assert.match(
      token,
      /^[0-9a-f]{64}$/,
      "raw token is 32 random bytes as hex",
    );

    const bad = await req("POST", "/api/auth/verify-email", {
      token: "f".repeat(64),
    });
    assert.equal(
      bad.status,
    ...
    const malformed = await req("POST", "/api/auth/verify-email", {
      token: "not-a-token",
    });
...
test("POST /me/resend-verification requires auth, is disabled while email is dormant", async () => {
  const clean = await fetch(base + "/api/me/resend-verification", {
    method: "POST",
  }); // no cookies
  assert.equal(clean.status, 401);
...
test("PATCH /me/profile succeeds for an unverified student while email is dormant (zero behavior change)", async () => {
  ...
  const reg = await req("POST", "/api/auth/register", {
    full_name: "Dormant Gate",
    email: testAddr,
    password: "password123",
    consent: true,
  });
  assert.equal(reg.json.student.email_verified, false);
  const patch = await req("PATCH", "/api/me/profile", {
    degree_level: "Master",
  });
  assert.equal(
    patch.status,
    200,
    "requireVerifiedEmail must be a no-op while email.ENABLED is false",
  );
  await req("DELETE", "/api/me");
});
...
test("forgot-password always responds 200, whether or not the account exists (no enumeration)", async () => {
  ...
  try {
    ...
    const real = await req("POST", "/api/auth/forgot-password", {
      email: testAddr,
    });
    ...
    const fake = await req("POST", "/api/auth/forgot-password", {
      email: `nobody_${Date.now()}@example.com`,
    });
    assert.equal(
      fake.status,
      200,
      "identical response for a non-existent account",
    );
    assert.equal(
      capturedLink,
      undefined,
      "no email/token generated for an account that does not exist",
    );
  } finally {
...
test("password reset: valid token changes the password and invalidates every existing session", async () => {
  ...
  try {
    ...
    const badPw = await req("POST", "/api/auth/reset-password", {
      token: resetToken,
      password: "short",
    });
    assert.equal(
      badPw.status,
      400,
      "new password still goes through the min-length rule",
    );
    ...
    const staleCheck = await fetch(base + "/api/auth/me", {
      headers: { Cookie: `uv_token=${staleToken}` },
    });
    assert.equal(staleCheck.status, 401);

    // Reused reset token is rejected (cleared on success).
    const reused = await req("POST", "/api/auth/reset-password", {
      token: resetToken,
...
test("change-email: full flow — requires correct password, resets verification, notifies the old address, invalidates other sessions", async () => {
  ...
  emailService.sendEmailChangedNotice = (name, o, n) => {
    noticeTo = o;
    noticeOld = o;
    noticeNew = n;
    return Promise.resolve({ sent: false });
  ...
  try {
    ...
    const wrongPw = await req("POST", "/api/me/change-email", {
      new_email: newAddr,
      password: "wrongpassword",
    });
    assert.equal(wrongPw.status, 401);

    const ok = await req("POST", "/api/me/change-email", {
      new_email: newAddr,
      password: "password123",
    });
    assert.equal(ok.status, 200);
    ...
    assert.equal(
      ok.json.student.email_verified,
      false,
      "the new address is unverified until proven",
    );
    ...
    assert.ok(
      verifyLink,
      "a fresh verification email was sent to the new address",
    );
    ...
    const staleCheck = await fetch(base + "/api/auth/me", {
      headers: { Cookie: `uv_token=${preChangeToken}` },
    });
    assert.equal(staleCheck.status, 401);
...
test("change-email requires authentication and rejects an email already in use", async () => {
  const clean = await fetch(base + "/api/me/change-email", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ new_email: "x@example.com", password: "x" }),
  });
  assert.equal(clean.status, 401);
  ...
  const clash = await req("POST", "/api/me/change-email", {
    new_email: addrA,
    password: "password123",
  });
...
test("POST /api/pilot-leads requires name, work email and university name", async () => {
  const missing = await req("POST", "/api/pilot-leads", {
    contact_name: "A B",
  });
...
test("the verified filter narrows results to the complete-profile tier", async () => {
  ...
  assert.ok(
    ver.json.count < all.json.count,
    "verified tier is a strict subset",
  );
