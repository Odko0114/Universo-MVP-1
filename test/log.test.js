...
function captureStream(stream) {
  ...
  const calls = [];
  stream.write = (chunk) => { calls.push(chunk); return true; };
  return { calls, restore: () => { stream.write = original; } };
}
...
test('debug is suppressed at the default (info) floor', () => {
  const out = captureStream(process.stdout);
  try { log.debug('should not appear'); } finally { out.restore(); }
  assert.equal(out.calls.length, 0);
});
...
test('captureError logs the message and a truncated stack, never throws on a non-Error', () => {
  ...
  try {
    ...
    assert.doesNotThrow(() => log.captureError('a string, not an Error object'));
  } finally { err.restore(); }
...
test('LOG_LEVEL=error raised in a fresh process suppresses info/warn', () => {
  ...
  const { stdout, stderr } = spawnSync(process.execPath, ['-e', script], { encoding: 'utf8' });
