...
test('rotateIfLarge archives an oversized file and leaves a clean live file', async () => {
  const original = fs.existsSync(events.FILE) ? fs.readFileSync(events.FILE) : null;
  ...
  try {
    ...
    assert.equal(fs.existsSync(events.FILE), false, 'live file should have been renamed away');

    const after = fs.readdirSync(require('node:path').dirname(events.FILE));
    const newFiles = after.filter((f) => !before.includes(f));
    assert.equal(newFiles.length, 1, 'exactly one archive file should have appeared');
    assert.match(newFiles[0], /events-archive-\d{4}-\d{2}-\d{2}(-\d+)?\.jsonl$/);

    // Clean up the archive this test created.
    fs.unlinkSync(require('node:path').join(require('node:path').dirname(events.FILE), newFiles[0]));
  } finally {
...
test('rotateIfLarge is a no-op when the file is under the threshold', async () => {
  const original = fs.existsSync(events.FILE) ? fs.readFileSync(events.FILE) : null;
  try {
    fs.writeFileSync(events.FILE, '{"ts":"2026-01-01T00:00:00.000Z","type":"pageview"}\n');
    const rotated = await events.rotateIfLarge();
