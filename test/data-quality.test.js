...
const FULL = {
  ...
  short_description: "A university.",
};
...
test("lastVerifiedAt uses real provenance, never invents a date", () => {
  assert.equal(
    dq.lastVerifiedAt({ source: "curated" }, "2026-07-16"),
    "2026-07-16",
  );
  assert.equal(
    dq.lastVerifiedAt({ source: "eter", ref_year: 2022 }),
    "2022-01-01",
  );
  assert.equal(
    dq.lastVerifiedAt({ source: "global" }),
    null,
    "no source date → null, not fabricated",
  );
});
...
test("isStale: an ETER record behind the latest available ref year is stale; at latest it is not", () => {
  ...
  assert.equal(
    dq.isStale({ source: "eter", ref_year: latest - 1 }),
    true,
    "a newer snapshot exists → refreshable",
  );
  assert.equal(
    dq.isStale({ source: "eter", ref_year: latest }),
    false,
    "already on the newest ETER snapshot",
  );
...
test("auditDataset aggregates distribution, status, staleness and missing counts", () => {
  ...
  assert.ok(
    Array.isArray(a.structurally_unavailable) &&
      a.structurally_unavailable.length > 0,
  );
  // most-missing field is ranked first
  assert.ok(
    a.missing_fields[0].missing >=
      a.missing_fields[a.missing_fields.length - 1].missing,
  );
});
...
test("performance: audits the full ~4,000-record dataset well under a second", () => {
  ...
  assert.ok(
    ms < 500,
    `audit took ${ms}ms — should be well under 500ms for one boot-time pass`,
  );
  assert.equal(a.count, list.length);
  // Every record got honest metadata during the build.
  assert.ok(
    list.every(
      (u) => u.data_source && u.verification_status && "last_verified_at" in u,
    ),
  );
});
