...
function makeReq(ip = '1.2.3.4') {
  return { method: 'POST', path: `/test-${n}`, headers: {}, socket: { remoteAddress: ip } };
}
function makeRes() {
...
test('allows up to max requests, then blocks with 429 + Retry-After', () => {
  ...
  const req = makeReq();
  for (let i = 0; i < 3; i++) assert.equal(run(limiter, req).passed, true, `request ${i + 1} should pass`);
...
test('the window slides: old hits expire and requests pass again', async () => {
  ...
  assert.equal(run(limiter, req).passed, true, 'should pass again after the window expires');
});
...
test('limits are per client ip — one abuser cannot exhaust another user\'s quota', () => {
  ...
  assert.equal(run(limiter, makeReq('8.8.8.8')).passed, true, 'a different ip still has quota');
});
...
test('uses the first X-Forwarded-For hop when behind a proxy', () => {
  n++;
  const limiter = rateLimit({ windowMs: 60_000, max: 1 });
  const forwarded = { method: 'POST', path: `/test-${n}`, headers: { 'x-forwarded-for': '7.7.7.7, 10.0.0.1' }, socket: { remoteAddress: '10.0.0.1' } };
  assert.equal(run(limiter, forwarded).passed, true);
  assert.equal(run(limiter, forwarded).passed, false, 'same forwarded ip shares one bucket');
});

test('supports a custom message', () => {
  n++;
  const limiter = rateLimit({ windowMs: 60_000, max: 0, message: 'Custom slow down' });
