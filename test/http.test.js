...
test('a successful first attempt calls fetch exactly once', async () => {
  let calls = 0;
  const restore = mockFetch(async () => { calls++; return { status: 200 }; });
  try {
...
test('throws the last error once retries are exhausted', async () => {
  const restore = mockFetch(async () => { throw new Error('always fails'); });
  try {
...
test('circuit breaker opens after 5 consecutive failures on the same host, then fails fast', async () => {
  const url = freshUrl();
  let calls = 0;
  const restore = mockFetch(async () => { calls++; throw new Error('down'); });
  try {
    for (let i = 0; i < 5; i++) {
    ...
    await assert.rejects(() => fetchWithResilience(url, { retries: 0 }), /circuit open/);
    assert.equal(calls, 5, 'fetch should not have been invoked while the circuit is open');
  } finally { restore(); }
});
...
test('a success clears the failure count so the breaker does not open early', async () => {
  ...
  try {
    ...
    for (let i = 0; i < 3; i++) await assert.rejects(() => fetchWithResilience(url, { retries: 0 }));
    ...
    await assert.rejects(() => fetchWithResilience(url, { retries: 0 }), (err) => !/circuit open/.test(err.message));
  } finally { restore(); }
});
