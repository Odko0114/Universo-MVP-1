...
test("registration caps overly long fields", () => {
  const r = validate.registration({
    ...
    password: "password1",
    consent: true,
    field_of_interest: "x".repeat(1000),
  });
...
test("registration records the separate product-updates opt-in (default false)", () => {
  ...
  assert.equal(
    validate.registration({ ...base, updates_optin: true }).value.updates_optin,
    true,
  );
  assert.equal(
    validate.registration({ ...base, updates_optin: "yes" }).value
      .updates_optin,
    false,
  ); // strict boolean only
});
...
test("pilotLead requires name, work email and university name", () => {
  ...
  assert.equal(
    validate.pilotLead({ contact_name: "A B", work_email: "a@b.com" }).ok,
    false,
  );
});
