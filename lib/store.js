...
// Fail fast in production if it's missing — a silent fallback here means
// every registration/click/account "succeeds" and then vanishes on the next
// deploy, with no error anywhere to point at why.
...
function read(name) {
  if (!cache.has(name)) {
    throw new Error(
      `Collection "${name}" not initialised. Call store.init() first.`,
    );
  }
  return cache.get(name);
}
...
function flushAll() {
  ...
  for (const name of dirty) {
    try {
      fs.writeFileSync(
        filePath(name),
        JSON.stringify(cache.get(name), null, 2),
      );
    } catch {
