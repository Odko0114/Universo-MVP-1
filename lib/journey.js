 ...
 * @param {string[]} [appStatuses]  per-application statuses that auto-light stages
...
// Shortlist priority buckets — the narrowing tool while a student is still
// deciding (status "planning"). "" = not yet sorted.
const PRIORITIES = [
  { key: "reach", label: "Reach" },
  { key: "target", label: "Target" },
  { key: "safety", label: "Safety" },
];
const PRIORITY_KEYS = new Set(PRIORITIES.map((p) => p.key));
// Older data used "likely" for what's now "safety".
const normalizePriority = (p) =>
  p === "likely" ? "safety" : PRIORITY_KEYS.has(p) ? p : "";
 ...
 * @returns {{status:string, deadline:string, program:string, intake:string, notes:string, decision_date:string, priority:string, reason:string, req:Record<string,string>, docs:Record<string,boolean>, custom:any[]}}
 */
 ...
 * @param {{id:string, name:string, country?:string, application_link?:string, website?:string, application_deadline?:string, acceptance_requirements?:string}} uni
...
function applicationView(appRaw, uni, vaultDocs = {}, budget = null) {
  ...
  const docs = DOCUMENTS.map((d) => {
    ...
    // Completion is INDEPENDENT per application — always the app's own state,
    // never the shared vault (checking a doc for one application must not tick
    // it for another). `have` is the recognition hint: the student possesses
    // this shared document on file (My Documents), so they can tick it here fast.
    const ready = app.docs[d.key] === true;
    const have = d.shared ? vaultDocs[d.key] === true : false;
    return { key: d.key, label: d.label, shared: d.shared, custom: false, level, ready, have, listed };
  });
 ...
 * @param {string[]} [inputs.scholarshipStatuses]   per-scheme statuses, if any
