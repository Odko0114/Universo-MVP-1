...
// brand (optional) lets Brand Brain + the chosen goal steer the output without
// touching the data logic: brand.cta replaces the closing CTA, brand.help
// replaces the "How Universo helps" line. Omitted -> the built-in defaults.
function composeScript(opts, data, brand) {
  const o = opts || {};
  ...
  brand = brand || {};
  if (brand.cta) out.cta = String(brand.cta).slice(0, 200);
  if (brand.help) { const b = out.beats.find((x) => x.cue === "How Universo helps"); if (b) b.line = clip(brand.help, 220); }
  return out;
}
...
if (require.main === module) {
  ...
  // brand (Brand Brain + goal) steers CTA and the "How Universo helps" line
  const branded = composeScript({ topic: "Scholarships", angle: "Hungary" }, { universities: unis, scholarships: schs }, { cta: "Join free today.", help: "Universo, in my brand words." });
  assert.equal(branded.cta, "Join free today.", "brand CTA overrides default");
  assert.ok(branded.beats.some((b) => b.cue === "How Universo helps" && b.line === "Universo, in my brand words."), "brand help line applied");
