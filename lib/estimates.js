...
function enrich(u) {
  ...
  if (!u.tuition_range) {
    u.tuition_range = { min: est.tuition[0], max: est.tuition[1], currency: 'EUR', period: 'year', estimated: true };
  }
  if (!u.estimated_living_cost) {
    u.estimated_living_cost = { min: est.living[0], max: est.living[1], currency: 'EUR', period: 'month', estimated: true };
  }
  if (!(u.language_of_instruction && u.language_of_instruction.length) && est.languages.length) {
