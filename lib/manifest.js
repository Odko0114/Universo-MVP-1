...
function assertQuality(
  records,
  { source, minCount, requireField, minFieldRatio = 0.9 },
) {
  if (records.length < minCount) {
    throw new Error(
      `[${source}] sanity check failed: got ${records.length} records, expected ≥ ${minCount}. Aborting so we don't ship broken data.`,
    );
  }
  const withField = records.filter(
    (r) => r[requireField] != null && r[requireField] !== "",
  ).length;
  const ratio = withField / records.length;
  if (ratio < minFieldRatio) {
    throw new Error(
      `[${source}] sanity check failed: only ${(ratio * 100).toFixed(1)}% of records have "${requireField}" (expected ≥ ${minFieldRatio * 100}%).`,
    );
  }
}
