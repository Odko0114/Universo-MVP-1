...
function query(index, params, clickOf = () => 0, opts = {}) {
  ...
  const keep = (u) => {
    ...
    if (
      (verified === "1" || verified === "true" || verified === true) &&
      !u.verified
    )
      return false;
    if (field && !(u.fields_of_study || []).includes(field)) return false;
    if (language && !(u.language_of_instruction || []).includes(language))
      return false;
    if (degree && !(u.degree_levels || []).includes(degree)) return false;
    if (
      maxTuition !== undefined &&
      maxTuition !== "" &&
      !Number.isNaN(Number(maxTuition))
    ) {
      if (!u.tuition_range || (u.tuition_range.min ?? 0) > Number(maxTuition))
        return false;
    }
    return true;
  };
  ...
  // Among universities with the SAME match score, surface the stronger one
  // first instead of whoever sorts earliest alphabetically. "Stronger" uses
  // real signals only: a better world rank wins, then a deeply verified profile
  // (curated, checked) over a bare register entry. Unranked universities sort
  // after ranked ones on a tie — we can't claim strength we don't have — but
  // are never excluded. This only ever breaks ties; it never overrides fit.
  const rankScore = (u) => {
    const r = u.ranking && u.ranking.world_rank;
    return typeof r === "number" && r > 0 ? r : Infinity;
  };
  const byStrength = (a, b) => {
    const ra = rankScore(a.e.u);
    const rb = rankScore(b.e.u);
    if (ra !== rb) return ra - rb; // lower world rank = stronger = first
    return (b.e.u.verified ? 1 : 0) - (a.e.u.verified ? 1 : 0); // verified first
  };
  ...
  if (sort === "match" && scoreFn) {
    ...
    switch (sort) {
      case "tuition":
        rows.sort((a, b) => {
          const at = a.e.u.tuition_range
            ? (a.e.u.tuition_range.min ?? 0)
            : Infinity;
          const bt = b.e.u.tuition_range
            ? (b.e.u.tuition_range.min ?? 0)
            : Infinity;
      ...
      case "popular":
        rows.sort(
          (a, b) => clickOf(b.e.u.id) - clickOf(a.e.u.id) || byName(a, b),
        );
        break;
      case "size":
        rows.sort(
          (a, b) =>
            (b.e.u.student_count || 0) - (a.e.u.student_count || 0) ||
            byName(a, b),
        );
        break;
      default:
  ...
  const page = rows
    ...
    .map((x) => ({ ...x.e.u, click_count: clickOf(x.e.u.id) }));
