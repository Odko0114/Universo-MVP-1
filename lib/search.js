 ...
 * @param {object} params    { q, country, region, type, source, field, language, degree, maxTuition, sort, offset, limit }
 * @param {(id:string)=>number} clickOf  click count lookup
 */
function query(index, params, clickOf = () => 0) {
  const { q, country, region, type, source, field, language, degree, maxTuition, sort } = params;
