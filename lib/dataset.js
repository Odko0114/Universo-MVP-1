...
function dedupeByNameCountry(merged) {
  ...
  for (const u of merged) {
    ...
    if (!cur || (SOURCE_RANK[u.source] ?? 9) < (SOURCE_RANK[cur.source] ?? 9))
      byKey.set(key, u);
  }
  const redirects = {};
  const kept = [];
...
function describeEter(u) {
  const kind =
    u.legal_status === "Public"
      ? "public"
      : u.legal_status
        ? u.legal_status.toLowerCase()
        : "";
  const type = (u.institution_type || "university").replace(
    "Other institution",
  ...
  s += ".";
  if (u.student_count)
    s += ` Home to around ${Number(u.student_count).toLocaleString("en-US")} students.`;
  return s;
}
...
function buildDataset() {
  ...
  // The "global" source (a worldwide name+country+website list) is deliberately
  // NOT loaded: every one of its ~936 European rows had only a website — no
  // city, coordinates, fields of study or degree levels — so it padded the
  // headline count with hollow, unmatchable, mostly-vocational stubs. Dropping
  // it means every university now comes from the official EU register (ETER) or
  // is hand-curated. Re-add specific institutions properly rather than the list.
  let merged = mergeSources([
    curatedTagged,
    loadJson("eter-universities.json"),
  ]);
  ...
  LAST_REDIRECTS = deduped.redirects;
  {
    ...
    for (const u of merged) {
      const key = `${normName(u.name)}|${u.country}`;
      if (seen.has(key))
        throw new Error(`[dataset] duplicate university after dedup: ${key}`);
      seen.add(key);
    }
  }
  ...
  // Fill two more gaps with REAL data (never guessed):
  //  • coordinates from a one-time geocoded city lookup (coords-overrides.json),
  //    for records that have a real city but no coords — city-level, so marked
  //    approx:true (honest "which city", not a fabricated campus pin);
  //  • website derived from a hand-verified application_link when the record
  //    carries no website of its own (curated records store the link, not a
  //    bare website field).
  const coordsOverrides = loadJson("coords-overrides.json");
  for (const u of merged) {
    if (!(u.coords && u.coords.lat != null) && u.city) {
      const hit = coordsOverrides[`${u.city}|${u.country}`];
      if (hit) u.coords = { lat: hit.lat, lon: hit.lon, approx: true };
    }
    if (!u.website && u.application_link) {
      try {
        u.website = new URL(u.application_link).origin;
      } catch {
        /* not a URL — leave blank rather than guess */
      }
    }
  }
  ...
  for (const u of merged) {
    u.tuition_source =
      u.source === "curated"
        ? "curated_research"
        : u.tuition_range && u.tuition_range.estimated
          ? "country_estimate"
          : u.tuition_range
            ? "source_data"
            : "unknown";
  ...
  for (const u of merged)
    u.scholarships = scholarships.scholarshipsFor(u.country);
