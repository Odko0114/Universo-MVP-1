...
function hardFilters(student, u) {
  ...
  if (student.budget_max_eur_year != null) {
    if (u.tuition_source === 'curated_research' && u.tuition_intl_min != null) {
      if (u.tuition_intl_min > student.budget_max_eur_year) return { passed: false, flags };
    } else {
  ...
  if (student.degree_level) {
    ...
    if (levels.length) { if (!levels.includes(student.degree_level)) return { passed: false, flags }; }
...
function softScore(student, u) {
  ...
  let score = 0;
  const add = (key, label, pts) => { score += pts; components.push({ key, label, pts }); };

  const want = student.fields_of_interest || [];
  const offered = u.fields_offered || [];
  const fieldHit = want.filter((f) => offered.includes(f));
  if (fieldHit.length) add('field', `Offers ${fieldHit.join(' & ')}`, WEIGHTS.field);

  if (student.city_preference && u.city_size_category && u.city_size_category === student.city_preference) {
    const label = { large: 'In a large city', mid: 'In a mid-size city', small: 'In a small town' }[u.city_size_category];
    add('city', label, WEIGHTS.city);
  }

  if ((student.country_preference || []).length && student.country_preference.includes(u.country)) {
    add('country', `In ${u.country} (a country you picked)`, WEIGHTS.country);
  }

  if (u.verified) add('verified', 'Verified, university-confirmed profile', WEIGHTS.verified);

  // Comfortably-under-budget bonus — confirmed tuition only, well below ceiling.
  if (student.budget_max_eur_year != null && u.tuition_source === 'curated_research' && u.tuition_intl_min != null) {
    if (u.tuition_intl_min <= student.budget_max_eur_year * 0.8) {
      add('budget', u.tuition_intl_min === 0 ? 'No tuition fee' : 'Comfortably within your budget', WEIGHTS.budget);
    }
  }
...
function matchUniversity(student, u) {
  ...
  return { passed: hard.passed, score: soft.score, components: soft.components, flags: hard.flags };
}
...
function recommend(student, universities, opts = {}) {
  ...
  scored.sort((a, b) => b.score - a.score || (b.u.verified ? 1 : 0) - (a.u.verified ? 1 : 0));
...
function scoreUniversity(student, u) {
  const m = matchUniversity(student, u);
  return { score: m.score, reasons: m.components.map((c) => c.label), components: m.components, flags: m.flags, passed: m.passed };
}

module.exports = { matchUniversity, recommend, scoreUniversity, hasProfile, WEIGHTS };
