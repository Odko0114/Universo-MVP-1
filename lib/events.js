...
// Parsed-log cache. The admin dashboard fires ~8 endpoints at once and each
// aggregation reads the whole log — that used to be ~8 synchronous full reads
// + re-parses per dashboard load. Cache the parsed array and only re-read when
// the file actually changes (mtime + size), so repeat reads within a load (and
// across refreshes with no new events) are free. Callers must treat the result
// as READ-ONLY — the pure aggregations only read, and computeRetention copies
// before sorting. A flush/rotate/purge changes the file's mtime, which
...
function readAll() {
  ...
  if (key === _cache.key) return _cache.data;
  const out = [];
...
// Actions that mean a signed-up person actually DID something with Universo,
// not just created an account and left.
const ACTIVATION_ACTIONS = ["save", "compare", "apply_click", "dream_update"];

/**
 * Activation: of the people who registered, how many took a real action
 * (saved / compared / applied / built a dream)?
 *
 * Honest floor, not a precise rate: it matches a signup to an action only when
 * both happened under the SAME anonymous id. Someone who signs up on their
 * phone and saves on their laptop has two ids, so they'd be counted as
 * not-activated here — this can only UNDER-count activation, never inflate it.
 */
function computeActivation(events) {
  const signupAnons = new Set();
  const actorAnons = new Set();
  for (const e of events) {
    if (!e.anon) continue;
    if (e.type === "signup") signupAnons.add(e.anon);
    if (ACTIVATION_ACTIONS.includes(e.type)) actorAnons.add(e.anon);
  }
  let activated = 0;
  for (const a of signupAnons) if (actorAnons.has(a)) activated++;
  return {
    signups: signupAnons.size,
    activated,
    rate: signupAnons.size
      ? Math.round((activated / signupAnons.size) * 1000) / 10
      : 0,
  };
}
