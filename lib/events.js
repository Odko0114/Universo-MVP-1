...
if (cfg.PROD && !process.env.UNIVERSO_DATA_DIR) {
  throw new Error('UNIVERSO_DATA_DIR must be set in production (point it at a persistent volume).');
}
...
const shouldRotate = (sizeBytes, threshold = ROTATE_AT_BYTES) => sizeBytes >= threshold;
...
async function rotateIfLarge() {
  ...
  let size;
  try { size = fs.statSync(FILE).size; } catch { return false; } // no file yet — nothing to rotate
  if (!shouldRotate(size)) return false;

  const stamp = new Date().toISOString().slice(0, 10);
  let archivePath = FILE.replace(/\.jsonl$/, `-archive-${stamp}.jsonl`);
  let n = 2;
  while (fs.existsSync(archivePath)) archivePath = FILE.replace(/\.jsonl$/, `-archive-${stamp}-${n++}.jsonl`);

  await fsp.rename(FILE, archivePath);
  log.info('events log rotated', { archivePath: path.basename(archivePath), bytes: size });
  return true;
}
...
const rotateTimer = setInterval(() => { rotateIfLarge().catch(() => {}); }, 60 * 60 * 1000);
rotateTimer.unref?.();
...
function record(type, meta = {}) {
  buffer.push(JSON.stringify({ ts: new Date().toISOString(), type, ...meta }) + '\n');
  scheduleFlush();
}
...
async function purgeAnon(anonIds) {
  ...
  const kept = lines.filter((line) => {
    ...
    let e;
    try { e = JSON.parse(line); } catch { return true; }
    if (e.anon && drop.has(e.anon)) { removed++; return false; }
    return true;
  });
...
function computeSummary(events, now = Date.now()) {
  ...
  for (const e of events) {
    ...
    if (e.type === 'apply_click' && e.uni) {
      ...
      (applyUniqueSets[e.uni] = applyUniqueSets[e.uni] || new Set()).add(e.anon || Math.random());
    }
  }
  const applyUnique = {};
  for (const [uni, set] of Object.entries(applyUniqueSets)) applyUnique[uni] = set.size;

  return { totals, last24h, last7d, applyByUni, applyUnique, count: events.length };
}
...
function computeTopByUni(events, type, limit = 10) {
  ...
  for (const e of events) {
    ...
    (uniqueSets[e.uni] = uniqueSets[e.uni] || new Set()).add(e.anon || Math.random());
  }
...
function computeTopSearches(events, opts = {}) {
  ...
  return [...counts.entries()].sort((a, b) => b[1] - a[1]).slice(0, limit).map(([q, count]) => ({ q, count }));
}
...
const FUNNEL_STAGES = [
  ...
  { key: 'search', label: 'Searched', types: ['search'] },
  { key: 'profile_view', label: 'Viewed a profile', types: ['profile_view'] },
  { key: 'save', label: 'Saved a university', types: ['save'] },
  { key: 'apply_click', label: 'Clicked Apply', types: ['apply_click'] },
];
...
function computeFunnel(events, opts = {}) {
  const { sinceMs } = opts;
  const now = Date.now();
  const inWindow = sinceMs ? events.filter((e) => now - Date.parse(e.ts) <= sinceMs) : events;

  const stageSets = FUNNEL_STAGES.map(() => new Set());
  for (const e of inWindow) {
    if (!e.anon) continue;
    FUNNEL_STAGES.forEach((stage, i) => { if (stage.types.includes(e.type)) stageSets[i].add(e.anon); });
  }
  ...
  return FUNNEL_STAGES.map((stage, i) => {
    ...
    const row = {
      ...
      pct_of_prev: prev == null ? 100 : (prev ? Math.round((n / prev) * 1000) / 10 : 0),
    };
    prev = n;
    return row;
  });
...
function computeRetention(events, { weeks = 6 } = {}) {
  ...
  const sorted = [...events].filter((e) => e.anon).sort((a, b) => Date.parse(a.ts) - Date.parse(b.ts));
  ...
  for (const e of sorted) {
    ...
    const weekIdx = Math.floor((isoWeekStart(new Date(t)).getTime() - originWeek) / WEEK_MS);
  ...
  for (const [anon, t] of firstSeen) {
    const w = Math.floor((isoWeekStart(new Date(t)).getTime() - originWeek) / WEEK_MS);
    if (!cohortMembers.has(w)) cohortMembers.set(w, []);
    cohortMembers.get(w).push(anon);
  }

  const nowWeek = Math.floor((isoWeekStart(new Date()).getTime() - originWeek) / WEEK_MS);
  const cohortWeeks = [...cohortMembers.keys()].sort((a, b) => a - b).filter((w) => nowWeek - w < weeks + 8); // cap history shown
  ...
  const cohorts = cohortWeeks.slice(-weeks).map((w) => {
    ...
    for (let offset = 0; offset < weeks; offset++) {
      ...
      if (targetWeek > nowWeek) { retention.push(null); continue; } // hasn't happened yet
      const returned = members.filter((a) => activeWeeks.get(a)?.has(targetWeek)).length;
      retention.push(Math.round((returned / members.length) * 1000) / 10);
    }
    return { week_start: weekDate.toISOString().slice(0, 10), size: members.length, retention };
  });
 ...
 */
function computeUniTimeseries(events, uniId, { days = 30, now = Date.now() } = {}) {
  const series = [];
  ...
  for (const e of events) {
    ...
    const key = e.type === 'profile_view' ? 'views' : e.type === 'save' ? 'saves' : e.type === 'apply_click' ? 'applies' : null;
    if (!key) continue;
...
function computeTraffic(events, opts = {}) {
  ...
  const viewEvents = inWindow.filter((e) => e.type === 'pageview' || e.type === 'profile_view');
  const uniqueVisitors = new Set(inWindow.map((e) => e.anon).filter(Boolean));

  const bump = (map, key) => { if (key) map.set(key, (map.get(key) || 0) + 1); };
  const paths = new Map(), referrers = new Map(), languages = new Map();
  ...
  for (const e of viewEvents) {
    ...
    device[e.device === 'mobile' || e.device === 'desktop' ? e.device : 'unknown']++;
  }
  const top = (map, limit = 8) => [...map.entries()].sort((a, b) => b[1] - a[1]).slice(0, limit).map(([k, v]) => ({ key: k, count: v }));
...
const uniTimeseries = (uniId, opts) => computeUniTimeseries(readAll(), uniId, opts);
...
module.exports = {
  record, readAll, flush, purgeAnon, rotateIfLarge, shouldRotate, FILE, ROTATE_AT_BYTES,
  summary, funnel, retention, traffic, topSearches, topByUni, uniTimeseries,
  // exported for pure unit tests
  computeSummary, computeFunnel, computeRetention, computeTraffic, computeTopSearches, computeTopByUni, computeUniTimeseries,
};
