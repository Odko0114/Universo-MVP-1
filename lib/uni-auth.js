...
function findByEmail(email) {
  return store.read('uni_accounts').find((a) => a.email === String(email || '').trim().toLowerCase());
}

function signToken(account) {
  return jwt.sign({ sub: account.account_id, role: 'university' }, cfg.JWT_SECRET, { expiresIn: `${TOKEN_TTL_DAYS}d` });
}
...
const setUniCookie = (res, account) => setCookie(res, COOKIE_UNI, signToken(account), TOKEN_TTL_DAYS);
...
function loadUniAccount(req) {
  ...
  return store.read('uni_accounts').find((a) => a.account_id === payload.sub) || null;
}
 ...
 */
async function createUniAccount({ email, password, university_id, university_name }) {
  const norm = String(email || '').trim().toLowerCase();
  if (!norm || !password || String(password).length < 10) {
    throw new Error('A valid email and a password of at least 10 characters are required.');
  }
  const accounts = store.read('uni_accounts');
  if (accounts.some((a) => a.email === norm)) throw new Error('An account with this email already exists.');
...
module.exports = { COOKIE_UNI, findByEmail, setUniCookie, clearUniCookie, loadUniAccount, requireUni, createUniAccount };
