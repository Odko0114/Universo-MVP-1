  ...
  Portugal: [
    {
      ...
      note: "Portugal has no single national degree scholarship — the Camões Institute funds language/culture and the FCT funds research (mainly PhD). Look for programme-specific or institutional aid for a full degree.",
    },
  ],
  Ireland: [
    {
  ...
  Czechia: [
    {
      ...
      note: "Government scholarships for students from designated developing countries only (a bilateral scheme) — check whether your country qualifies.",
    },
  ],
  Greece: [
    {
      ...
      note: "IKY (Greek State Scholarships Foundation) mainly funds Greek nationals and specific bilateral/exchange schemes — limited general options for international degree students.",
    },
  ],
  Slovakia: [
    {
      ...
      note: "The National Scholarship Programme mainly funds short study/research STAYS (mobility), not full degrees, plus some bilateral agreements — check the current call.",
    },
  ],
  Slovenia: [
    {
      ...
      note: "Ad Futura mainly funds mobility/short stays and Slovenian citizens studying abroad — limited options for incoming full-degree students.",
    },
  ],
  Croatia: [
    {
  ...
  Romania: [
    {
      ...
      note: "Awarded under bilateral government agreements — availability depends on an agreement with your country; check via your Ministry of Education.",
    },
  ],
  Lithuania: [
    {
      ...
      note: "State scholarships and Baltic/bilateral exchange schemes — availability depends on your country's agreements; check the current call.",
    },
  ],
  "United Kingdom": [
    {
...
// The full browsable catalog for the Scholarships page: EU-wide schemes, every
// host-country scheme grouped by destination, and home-country outbound schemes.
// Same honest pointers used everywhere else — each carries verify:true + a key.
function catalog() {
  const tag = (s, country, scope) => ({
    ...s,
  ...
  const grouped = (map, scope) =>
    Object.entries(map)
      .map(([country, list]) => ({
        country,
        scholarships: list.map((s) => tag(s, country, scope)),
      }))
      .sort((a, b) => a.country.localeCompare(b.country));
  return {
    eu_wide: EU_WIDE.map((s) => tag(s, "", "eu-wide")),
    destinations: grouped(COUNTRY_SCHOLARSHIPS, "country"),
    outbound: grouped(OUTBOUND_SCHOLARSHIPS, "home"),
  };
}

// Look up a single scheme across the whole catalog by its slug key.
function findByKey(key) {
  const c = catalog();
  const all = [
    ...c.eu_wide,
    ...c.destinations.flatMap((g) => g.scholarships),
    ...c.outbound.flatMap((g) => g.scholarships),
  ];
