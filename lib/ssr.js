...
// schema.org JSON-LD for a university profile (only emitted on verified/indexed
// pages by the caller). Values are JSON-encoded and `<` is escaped so a name
// can't break out of the <script> block.
function structuredData(u, url) {
  const data = { "@context": "https://schema.org", "@type": "CollegeOrUniversity", name: u.name, url };
  if (u.short_description) data.description = u.short_description;
  if (u.city || u.country) data.address = { "@type": "PostalAddress", addressLocality: u.city || undefined, addressCountry: u.country || undefined };
  if (u.website) data.sameAs = [u.website];
  return `<script type="application/ld+json">${JSON.stringify(data).replace(/</g, "\\u003c")}</script>`;
}

module.exports = { esc, metaTags, injectSSR, profileView, directoryView, structuredData };
