...
/** @param {{ title:string, description:string, canonical?:string, image?:string }} opts */
function metaTags({ title, description, canonical, image }) {
  const t = esc(title);
...
function profileView(u) {
  ...
  if (t) facts.push([`Tuition (intl)${u.tuition_range.estimated ? ', est.' : ''}`, u.tuition_range.estimated ? `~${t}` : t]);
...
function directoryView(list, total) {
  return `
    <section class="ssr">
      <h1>Discover universities abroad</h1>
      <p>Browse ${Number(total).toLocaleString('en-US')} universities worldwide by country, type, field of study and budget.</p>
