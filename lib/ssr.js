...
/** @param {{ title:string, description:string, canonical?:string, image?:string, noindex?:boolean }} opts */
...
function fmtMoney(r) {
  ...
  return r.min === r.max ? `${f(r.min)}/${r.period}` : `${f(r.min)}–${f(r.max)}/${r.period}`;
}
...
function profileView(u) {
  ...
  if (u.ranking && u.ranking.world_rank) {
    const nat = u.ranking.national_rank ? ` · #${u.ranking.national_rank} national` : '';
    facts.push(['Ranking', `#${u.ranking.world_rank} world${nat} (${u.ranking.provider})`]);
  }
  // Figures only for hand-researched tuition; estimate ranges display as a
  // pointer to the official site (see lib/dataset.js tuition_source).
  const t = fmtMoney(u.tuition_range);
  if (t && u.tuition_source === 'curated_research') facts.push(['Tuition (intl)', t]);
  else if (u.website) facts.push(['Tuition (intl)', `<a href="${esc(u.website)}" rel="noopener">Check official site</a>`]);
  if ((u.language_of_instruction || []).length) facts.push(['Language', u.language_of_instruction.join(', ')]);
  ...
  if (u.student_count) facts.push(['Students', `${Number(u.student_count).toLocaleString('en-US')} enrolled`]);
  ...
  return `
    <article class="ssr">
      ...
      ${u.acceptance_requirements ? `<h2>Admission requirements</h2><p>${esc(u.acceptance_requirements)}</p>` : ''}
