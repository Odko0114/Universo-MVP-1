...
const LLM_KEY = process.env.UNIVERSO_LLM_KEY || process.env.ANTHROPIC_API_KEY || '';
...
function tags(components) {
  return (components || []).map((c) => {
    switch (c.key) {
      case 'field': return c.label.replace(/^Offers /, '');       // "Computer Science"
      case 'city': return c.label.replace(/^In a |^In an /, '');   // "mid-size city"
      case 'country': return c.label.replace(/ \(a country you picked\)$/, '');
      case 'verified': return 'Verified profile';
      case 'budget': return c.label;                               // "In your budget" / "No tuition fee"
...
function structuredLine(components, flags = []) {
  ...
  const flagNote = flags.includes('tuition_unconfirmed') ? ' Tuition isn’t confirmed for this university yet — check the official site.' : '';

  if (t.length === 0) {
    return 'No strong matches to your profile yet — the details below can still help you compare.' + flagNote;
  }
  if (t.length === 1) {
...
const keyFor = (student, uni, components) =>
  crypto.createHash('sha1')
    .update(JSON.stringify([
      student.fields_of_interest, student.degree_level, student.budget_max_eur_year,
      student.preferred_languages, student.city_preference, student.country_preference,
      uni.id, components.map((c) => c.key),
    ]))
    .digest('hex');
...
module.exports = { tags, structuredLine, compressedReason, generate, LLM_ENABLED };
