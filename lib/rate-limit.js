 ...
 *
 * NOTE: counters live in process memory, so they RESET ON RESTART/REDEPLOY — a
 * determined attacker could reset their budget by waiting for a deploy. That's
 * an accepted tradeoff for a zero-cost, zero-dependency limiter at this stage.
 *
 * Usage:
...
setInterval(() => {
  ...
  for (const [k, hits] of buckets) {
    if (!hits.length || now - hits[hits.length - 1] > SWEEP_MS)
      buckets.delete(k);
  }
}, SWEEP_MS).unref();
