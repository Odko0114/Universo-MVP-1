 ...
 *      as "missing" on 3,900+ records would be dishonest noise, so they are NOT
...
const DIMENSIONS = [
  ...
  { key: 'founded', label: 'Founding year', weight: 8, has: (u) => u.founded != null },
  { key: 'student_count', label: 'Enrolment', weight: 10, has: (u) => u.student_count != null },
  { key: 'institution_type', label: 'Institution type', weight: 8, has: (u) => !!u.institution_type },
  { key: 'degree_levels', label: 'Degree levels', weight: 12, has: (u) => Array.isArray(u.degree_levels) && u.degree_levels.length > 0 },
  { key: 'fields_of_study', label: 'Fields of study', weight: 12, has: (u) => Array.isArray(u.fields_of_study) && u.fields_of_study.length > 0 },
  { key: 'language', label: 'Language of instruction', weight: 10, has: (u) => Array.isArray(u.language_of_instruction) && u.language_of_instruction.length > 0 },
  { key: 'coords', label: 'Coordinates', weight: 6, has: (u) => !!(u.coords && u.coords.lat != null && u.coords.lon != null) },
  { key: 'description', label: 'Description', weight: 6, has: (u) => !!u.short_description },
];
...
const STRUCTURALLY_UNAVAILABLE = [
  'per-university scholarships', 'application deadlines', 'admission requirements',
  'contact email/phone', 'social links', 'accommodation', 'campus description', 'programme catalogue',
];
...
function dataSourceLabel(source) {
  switch (source) {
    case 'curated': return 'Universo curated research';
    case 'eter': return 'ETER — official EU register';
    case 'global': return 'Hipolabs global university list';
...
function isStale(lastVerified, now = new Date()) {
  if (!lastVerified) return true; // no verification date at all → needs review
  const then = new Date(lastVerified);
  if (Number.isNaN(then.getTime())) return true;
  return (now.getTime() - then.getTime()) > STALE_YEARS * 365 * 24 * 60 * 60 * 1000;
}
...
function auditDataset(list) {
  const n = list.length;
  const distribution = { Excellent: 0, Good: 0, 'Needs Improvement': 0, Incomplete: 0 };
  ...
  for (const u of list) {
    ...
    scoreSum += score;
    distribution[band] += 1;
    byStatus[u.verification_status] = (byStatus[u.verification_status] || 0) + 1;
  ...
  const missingRanked = DIMENSIONS
    .map((d) => ({ key: d.key, label: d.label, missing: missingCounts[d.key], pct: n ? Math.round((100 * missingCounts[d.key]) / n) : 0 }))
...
module.exports = {
  DIMENSIONS, STRUCTURALLY_UNAVAILABLE, STALE_YEARS,
  scoreRecord, bandFor, dataSourceLabel, verificationStatus, lastVerifiedAt, isStale, auditDataset,
};
