...
function findAdminByEmail(email) {
  return store.read('admins').find((a) => a.email === String(email || '').trim().toLowerCase());
}

function signAdminToken(admin) {
  return jwt.sign({ sub: admin.admin_id, role: 'admin' }, cfg.JWT_SECRET, { expiresIn: `${cfg.ADMIN_TOKEN_TTL_DAYS}d` });
}
...
function setAdminCookie(res, admin) {
  setCookie(res, cfg.COOKIE_ADMIN, signAdminToken(admin), cfg.ADMIN_TOKEN_TTL_DAYS);
}
...
async function createAdmin(email, password) {
  ...
  if (existing) {
    ...
    admins.push({ admin_id: crypto.randomUUID(), email: norm, password_hash, created_at: new Date().toISOString() });
  }
...
module.exports = {
  findAdminByEmail, setAdminCookie, clearAdminCookie, loadAdmin, requireAdmin, createAdmin, bootstrapFromEnv,
};
