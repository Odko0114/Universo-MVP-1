...
function matchProfileFields(body = {}) {
  ...
  const fields_of_interest = arr(body.fields_of_interest)
    .filter((f) => FIELD_SET.has(f)).slice(0, 3);
  const preferred_languages = arr(body.preferred_languages)
    .map((l) => str(l)).filter((l) => l && l.length <= 40).slice(0, 6);
  const country_preference = arr(body.country_preference)
    .map((c) => str(c).slice(0, MAX.country)).filter(Boolean).slice(0, 10);
...
const validate = {
  ...
  dream(body = {}) {
    ...
    const target_intake = /^(Fall|Spring|Summer|Winter) \d{4}$/.test(intakeRaw) ? intakeRaw : '';
    return pass({
      target_intake,
  ...
  changeEmail(body = {}) {
    ...
    const password = typeof body.password === 'string' ? body.password : '';
    if (!EMAIL_RE.test(new_email) || new_email.length > MAX.email) return fail('A valid new email address is required.');
    if (!password) return fail('Your current password is required to change your email.');
