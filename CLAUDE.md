...
2026-08-09 | Item 5 | Continue card on Discover — "Home" in the task means Discover, since "/" 301s there and that's where returning visitors land. Reads Task 4's localStorage value, so it works for anonymous visitors too (most traffic), and expires after 30 days rather than offering to resume something months old. Progress line (profile % + saved count) only renders for signed-in students; profile_completeness now comes from publicStudent via journey.profileCompleteness, the same function Dream Plan's readiness card uses, so the two can't disagree. Fires a "continue" event. Found the task's progress-indicator requirement already satisfied on Dream Plan, so this is a compact summary linking there, not a second implementation. No streaks/badges/notifications added.
