...
2026-08-09 | Item 5 | REDEFINED after Odko clarified: this is navigation/state restoration, not a resume card. Removed the "pick up where you left off" card shipped earlier (wrong reading of the task file) along with its event type and the profile_completeness field added for it. Real fix is one line: render() called window.scrollTo(0, 0) on EVERY render including popstate, so Back reset the offset. Now suppressed for back/forward, leaving the browser's own scrollRestoration ("auto") to do the job. Filters/search/page were already restored by Item 4. NOTE: this un-defers "scroll restoration" from the deferred list, at Odko's explicit request.
