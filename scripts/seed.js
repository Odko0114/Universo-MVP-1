...
function run() {
  ...
  if (fs.existsSync(target) && !force) {
    console.log(
      `[seed] ${path.relative(process.cwd(), target)} already exists — skipping (use --force to overwrite).`,
    );
    return;
  }

  fs.writeFileSync(target, JSON.stringify(seed, null, 2));
  const by = seed.reduce(
    (m, u) => (
      (m[u.source || "curated"] = (m[u.source || "curated"] || 0) + 1),
      m
    ),
    {},
  );
  const parts = ["curated", "eter", "global"]
    .filter((s) => by[s])
    .map((s) => `${by[s]} ${s}`)
    .join(" + ");
  console.log(
    `[seed] Wrote ${seed.length} universities (${parts}) to ${path.relative(process.cwd(), target)}${force ? " (forced overwrite)" : ""}.`,
  );
}
