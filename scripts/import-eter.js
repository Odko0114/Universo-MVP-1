...
function fieldsOfStudy(r) {
  ...
  return FOE_CODES.filter((c) => totals[c] >= threshold)
...
function describe(rec) {
  ...
  s += ".";
  if (rec.student_count)
    s += ` Around ${rec.student_count.toLocaleString("en-US")} students enrolled.`;
  return s;
}
...
async function run() {
  ...
  for (const r of rows) {
    ...
    if (
      (domain && curatedDomains.has(domain)) ||
      (nm && curatedNames.has(nm))
    ) {
      skippedDup++;
      continue;
    }
    ...
    const founded = Number.isFinite(r["BAS.FOUNDYEAR"])
      ? r["BAS.FOUNDYEAR"]
      : null;
    const students =
      Number.isFinite(r["STUD.TOTALISCED5_7"]) && r["STUD.TOTALISCED5_7"] > 0
        ? Math.round(r["STUD.TOTALISCED5_7"])
        : null;
    ...
    const rec = {
      ...
      domain,
      institution_type:
        STAND_TYPE[r["BAS.INSTCATSTAND"]] || "Other institution",
  ...
  const target = path.join(
    ...
    "data",
    "seed",
    "eter-universities.json",
  );
  fs.writeFileSync(target, JSON.stringify(out, null, 2));
  manifest.write("eter", out, { ref_year: YEAR, source_api: API });

  console.log(
    `[import-eter] Wrote ${out.length} records to ${path.relative(process.cwd(), target)}`,
  );
  console.log(
    `[import-eter]   deduped against curated: ${skippedDup} | skipped (no id/name): ${skippedNoName}`,
  );
  const byType = out.reduce(
    (m, r) => ((m[r.institution_type] = (m[r.institution_type] || 0) + 1), m),
    {},
  );
  console.log("[import-eter]   by type:", JSON.stringify(byType));
  console.log(
    `[import-eter]   countries: ${new Set(out.map((r) => r.country)).size}`,
  );
}
