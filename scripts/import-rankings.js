...
const SOURCE = process.env.RANKINGS_URL
...
async function run() {
  ...
  const iName = col('institution'), iWorld = col('world_rank'), iNat = col('national_rank'), iCountry = col('country'), iYear = col('year');
  ...
  const out = {};
  for (const [key, v] of byName) out[key] = { world_rank: v.world_rank, national_rank: v.national_rank, country: v.country, year: v.year };

  const records = Object.values(out);
  manifest.assertQuality(records, { source: 'rankings', minCount: 800, requireField: 'world_rank' });
  ...
  console.log(`[import-rankings] Wrote ${records.length} ranked institutions to ${path.relative(process.cwd(), target)}`);
  console.log(`[import-rankings]   countries: ${new Set(records.map((r) => r.country)).size}`);
}

run().catch((e) => { console.error('[import-rankings] FAILED:', e.message); process.exit(1); });
